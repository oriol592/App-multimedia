...
import com.example.cep.appmultimedia.videoTrimmer.HgLVideoTrimmer;
...
public class TrimmerActivity extends AppCompatActivity implements OnTrimVideoListener, OnHgLVideoListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (extraIntent != null) {
            path = extraIntent.getStringExtra(ActivityMainRetallarVideo.EXTRA_VIDEO_PATH);
            maxDuration = extraIntent.getIntExtra(ActivityMainRetallarVideo.VIDEO_TOTAL_DURATION, 10);
        }
    ...
    public void getResult(final Uri contentUri) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
               Toast.makeText(TrimmerActivity.this, getString(R.string.video_saved_at, contentUri.getPath()), Toast.LENGTH_LONG).show();
